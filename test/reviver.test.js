 ...
 * @throws {Error} If the entity is unknown
 */
 ...
 * @param {string} entity - The name of the entity
 * @returns {object} The properties of the entity
 */
...
describe('Reviver', function() {
	...
	describe('#revive', function() {
		it('preserve unknown properties (properties are not checked)', function() {
			...
			assert.strictEqual(revived_city_1.streets[0].className, 'Street', 'Entity property for first street is "Street" because preserveEntityProperty has been set to true');
			assert.strictEqual(revived_city_1.streets.length, 2, 'Arrays are not modified: there is 2 streets');
		...
		it('does not preserve unknown properties (properties are checked and only declared properties are imported)', function() {
			const reviver = new Reviver({
				entityProperty: 'className',
				factory: buildEntity,
				entitiesProperties: getPropertiesForEntity,
